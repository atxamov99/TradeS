...
const sendSupportMessage = async ({ name, contact, message }, userId) => {
  ...
  if (ADMIN_SUPPORT_CHAT_ID) {
    ...
    // Best-effort — don't fail the user-facing request if Telegram delivery
    // hiccups; the message is still saved and can be checked manually.
    await telegramService.sendMessage(ADMIN_SUPPORT_CHAT_ID, text).catch(() => {});
  }
